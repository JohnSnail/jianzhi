...
- (id)initWithFrame:(CGRect)frame dataSource:(id)datasource delegate:(id) delegate
{
    ...
    if (self) {
        ...
        for (int i = 0; i <sectionNum; i++) {
            ...
            [sectionBtn setTitleColor:RGB(106, 112, 121) forState:UIControlStateNormal];
            sectionBtn.titleLabel.font = [UIFont boldSystemFontOfSize:13.0f];
            [self addSubview:sectionBtn];
            
            UIImageView *sectionBtnIv = [[UIImageView alloc] initWithFrame:CGRectMake(sectionWidth*i +(sectionWidth - 20), (self.frame.size.height-6)/2, 12, 6)];
...
-(void)showChooseListViewInSection:(NSInteger)section choosedIndex:(NSInteger)index
{
    if (!self.mTableView) {
        ...
        self.mTableView = [[UITableView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + self.frame.size.height, mainscreenwidth, mainscreenhight) style:UITableViewStylePlain];
    ...
    //动画设置位置
    rect .size.height = mainscreenhight-150;
    [UIView animateWithDuration:0.3 animations:^{
...
{
    static NSString *notice=@"TypeCell";
    TypeCell *cell=[tableView dequeueReusableCellWithIdentifier:notice];
    if (!cell) {
        cell =[[[NSBundle mainBundle] loadNibNamed:@"TypeCell" owner:self options:nil] lastObject];
        cell.selectionStyle=UITableViewCellSelectionStyleNone;
    }
    cell.nameLable.text = [self.dropDownDataSource titleInSection:currentExtendSection index:indexPath.row];
    cell.headerImage.image=[UIImage imageNamed:[self.dropDownDataSource titleInSection:currentExtendSection index:indexPath.row]];
