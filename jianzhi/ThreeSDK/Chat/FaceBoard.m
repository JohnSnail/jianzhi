...
- (id)init
{
    self = [super initWithFrame:CGRectMake(0, 0, 320*VIEWWITH, 216)];
    if (self) {
        ...
        //表情盘
        faceView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, 320*VIEWWITH, 190)];
        faceView.pagingEnabled = YES;
        faceView.contentSize = CGSizeMake((85/28+1)*320*VIEWWITH, 190);
        ...
        for (int i = 1; i<=85; i++) {
            ...
            //计算每一个表情按钮的坐标和在哪一屏
            faceButton.frame = CGRectMake(((((i-1)%28)%7)*44+6+((i-1)/28*320))*VIEWWITH, ((((i-1)%28)/7)*44+8)*VIEWWITH, 44, 44);
            
            [faceButton setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%d",i]] forState:UIControlStateNormal];
            [faceView addSubview:faceButton];
        }
        
        //添加PageControl
        facePageControl = [[GrayPageControl alloc]initWithFrame:CGRectMake(110*VIEWWITH, 190, 100, 20)];
        ...
        back.frame = CGRectMake(270*VIEWWITH, 185, 38, 27);
...
- (void)pageChange:(id)sender {
    [faceView setContentOffset:CGPointMake(facePageControl.currentPage*320*VIEWWITH, 0) animated:YES];
