...
-(void)getlocation
{
    ...
    self.lat=self.map.userLocation.coordinate.latitude;
    self.lng=self.map.userLocation.coordinate.longitude;
...
- (void)viewDidLoad
{
    ...
    //[self addBackButton];
    self.navigationItem.leftBarButtonItem=[LMButton setNavleftComboButtonWithImg:@"back" andSelector:@selector(backNow) andTarget:self];
    NSString *titStr;
    ...
    lb.textColor = [UIColor whiteColor];//SYSTEM_NAVIBAR_COLOR;
    lb.text = titStr;
    self.navigationItem.titleView = lb;
    
    self.regionMapView = [[MKMapView alloc] initWithFrame:CGRectMake(0, 0, mainscreenwidth, mainscreenhight)];
    ...
    goUserLocBtn.frame =CGRectMake(8,mainscreenhight-130, 40*VIEWWITH, 40*VIEWWITH); //FRAME_USER_LOC;
