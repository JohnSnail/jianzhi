...
-(void)nextStep
{
        ...
        if ([code1 isEqualToString:[_areaCodeField.text stringByReplacingOccurrencesOfString:@"+" withString:@""]])
        {
            ...
            if (!isMatch)
            {
                ...
                UIAlertView* alert=[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"温馨提示", nil)
                                                              message:NSLocalizedString(@"号码错误", nil)
                                                             delegate:self
    ...
    NSString* str=[NSString stringWithFormat:@"%@:%@ %@",NSLocalizedString(@"是否发送", nil),self.areaCodeField.text,self.telField.text];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (1==buttonIndex)
    {
        ...
        verify.hidesBottomBarWhenPushed = YES;
...
//                }];
            ...
            {
                UIAlertView* alert=[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"错误提示", nil)
...
- (void)viewDidLoad
{
    ...
    areaCodeField.frame=CGRectZero;
    ...
    telField.frame=CGRectMake(10, 35+statusBarHeight,mainscreenwidth - 20, 40+statusBarHeight/4);
    telField.borderStyle=UITextBorderStyleBezel;
    telField.placeholder=NSLocalizedString(@"请输入您的手机号", nil);
    ...
    nextBtn.frame=CGRectMake(10, 100+statusBarHeight, self.view.frame.size.width - 20, 42);
